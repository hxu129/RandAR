...
CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=29500 $PROJECT_DIR/tools/search_cfg_weights_corrector.py \
    --config "${PROJECT_DIR}/configs/randar/randar_${MODEL_SIZE_LATTER}_${MODEL_SIZE}_${TOKENIZER}.yaml" \
    --exp-name $MODEL_NAME-test-transformer \
    ...
    --corrector-ckpt "${DATA_PATH}/corrector-results/test-transformer_bs_512_lr_0.0001/checkpoints/iters_00004500/model.safetensors" \
