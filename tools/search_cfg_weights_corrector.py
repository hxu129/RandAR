...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    ...
    file_paths = glob.glob(f"{sample_dir}/*.png")
    
    # Optional: if you need to ensure a specific order, you can sort the file paths
    # file_paths.sort() 
    
    # Ensure we don't try to read more files than exist
    num_to_process = min(num, len(file_paths))

    for i in tqdm(range(num_to_process), desc="Building .npz file from samples"):
    ...
    assert samples.shape == (num_to_process, samples.shape[1], samples.shape[2], 3)
...
def sample_and_eval(tokenizer, gpt_model, corrector_model, cfg_scale, args, device, total_samples, corrector_num_ar_layers):
    # Setup DDP:
    ...
    for _ in pbar:
        ...
        indices = gpt_model.generate_with_corrector(
            ...
            corrector_num_ar_layers=corrector_num_ar_layers
        )
...
def main(args):
    ...
    # create and load corrector model
    corrector_config = OmegaConf.load(args.corrector_config)
    corrector_model = instantiate_from_config(corrector_config.corrector_model).to(device=device, dtype=precision).eval()
